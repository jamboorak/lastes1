...
$userAccounts = $conn->query("SELECT id, fullname, email FROM user_accounts LIMIT 5");
if ($userAccounts && $userAccounts->num_rows > 0) {
    echo "<strong>User accounts table:</strong><br>";
    while ($user = $userAccounts->fetch_assoc()) {
        echo "ID: {$user['id']} - {$user['fullname']} ({$user['email']})<br>";
    }
} else {
    echo "❌ No users found in user_accounts table<br>";
}
...
echo "<a href='reservations.php'>Try Reservations Page</a>";
?>
