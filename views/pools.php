...
?>
                ...
                <div class="pool-slider" id="poolSlider" style="display: flex; align-items: center; justify-content: center; gap: 3rem; position: relative;">
                    ...
                    <div class="slider-stage" style="width: 100%; max-width: 760px; height: 420px; position: relative; transform-style: preserve-3d; transition: transform 0.8s ease;">
                        <?php if (!empty($publicPools)): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div style="padding: 2rem; text-align: center; color: var(--text-light);">No pools available yet.</div>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        window.addEventListener('click', function(event) {
            const modal = document.getElementById('descriptionModalPool');
            if (event.target === modal) {
                modal.style.display = 'none';
            }
        });

        document.addEventListener('DOMContentLoaded', updatePoolRotation);
